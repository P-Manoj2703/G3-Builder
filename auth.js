...
const G3Auth = (() => {
  ...
  async function login(username, password) {
    ...
    // Fetch users from sheet — same pattern as loadData() in other pages
    ...
    // Check password — handle both string and numeric passwords from sheet
    const sheetPass   = String(user.password).trim().replace(/\.0+$/, '');
    const enteredPass = String(password).trim();
    if (sheetPass !== enteredPass) {
      return { ok: false, error: 'Invalid username or password.' };
    }
